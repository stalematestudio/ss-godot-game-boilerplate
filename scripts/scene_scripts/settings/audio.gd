...
onready var audio_devices = AudioServer.get_device_list()

func _ready():
	gui_playback.connect("item_selected", self, "set_playback_device")

func set_form_values():
	Helpers.RemoveChildren(gui_audio_vbc)
	for key in ConfigManager.config_data.audio.keys():
		var audio_bus = bus.instance()
		audio_bus.name = key
...
func set_playback_device(new_val):
	AudioServer.set_device(audio_devices[new_val])
